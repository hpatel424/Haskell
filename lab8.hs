...
compl (qs1, s1, fs1, d1) = (qs2, s2, fs2, d2) where
  qs2 = qs1
  s2  = s1
  fs2 = [q | q <- qs2, not (elem q fs1)]
  d2 = d1
...
quot_right ws (qs1, s1, fs1, d1) = (qs, s, fs, d) where
  qs = qs1
  s = s1
  fs = [q | q <- qs1, w <- ws, elem ((star d1) q w) fs1]
  d q a = d1 q a
...
nap_hat d qs a = norm $ concat[d q' a | q <- (power qs), q' <- q]
...
nacc (qs1, s1, fs1, d1) w = overlap (nap_hat_star d1 s1 w) fs1
...
eacc (qs, s, fs, d, es) [w] = overlap fs (concat (norm [eap_hat (d, es) qs w | q <- s]))
...
efsm_to_fsm (qs1, s1, fs1, d1, es1) = (qs, s, fs, d) where
 qs = [x | x <- power qs1, (eclose es1 x) == x]
 s = eclose es1 s1
 fs = [x | x <- qs, overlap x fs1]
